...
namespace TF_VIP_FD_Grab
{
    public partial class Main_Form : Form
    {
        ...
        private string __brand_color = "#9A0100";
        ...
        private void InitializeChromium()
        {
            ...
            chromeBrowser = new ChromiumWebBrowser("http://www.huatchaiya.com/page/manager/login.jsp");
        ...
        private void ChromiumBrowserAddressChanged(object sender, AddressChangedEventArgs e)
        {
            ...
            if (e.Address.ToString().Equals("http://www.huatchaiya.com/page/manager/login.jsp"))
            {
                if (__isLogin)
                {
                    Invoke(new Action(() =>
            ...
            if (e.Address.ToString().Equals("http://www.huatchaiya.com/page/manager/payment/deposit.jsp") || e.Address.ToString().Equals("http://www.huatchaiya.com/page/manager/dashboard.jsp") || e.Address.ToString().Equals("http://www.huatchaiya.com/page/manager/member/search.jsp"))
            {
        ...
        {
            IntPtr windowPtr = FindWindowByCaption(IntPtr.Zero, "JavaScript Alert - http://www.huatchaiya.com");
                ...
                else
                {
                    ...
                    if (player_info.Count != 0)
                    {
                        ...
                        foreach (string value in values)
                        {
                            ...
                            // ----- Insert Data
                            using (StreamWriter file = new StreamWriter(Path.GetTempPath() + @"\fdgrab_tfvip.txt", true, Encoding.UTF8))
                            {
        ...
        private async Task ___SearchPendingAsync(string bill_no)
        {
            try
            {
                ...
                byte[] result = await wc.DownloadDataTaskAsync("http://www.huatchaiya.com/manager/payment/searchDeposit?transactionId=" + bill_no + "&referenceNo=&userId=&status=9999&type=2&toBankIdOrBranch=-1&createDateStart=" + start_time + "&createDateEnd=" + end_time + "&vipLevel=-1&approvedDateStart=&approvedDateEnd=&pageNumber=1&pageSize=10&sortCondition=4&sortName=createTime&sortOrder=1&searchText=");
                string responsebody = Encoding.UTF8.GetString(result);
                var deserializeObject = JsonConvert.DeserializeObject(responsebody);
        ...
        private async Task ___PlayerListContactNumberAsync(string username, string type)
        {
            try
            {
                if (type == "normal")
                {
                    ...
                    byte[] result = await wc.DownloadDataTaskAsync("http://www.huatchaiya.com/manager/member/getProfileOverview?userId=" + username);
                    string responsebody = Encoding.UTF8.GetString(result);
                    var deserializeObject = JsonConvert.DeserializeObject(responsebody);
        ...
        private void SendMyBot(string message)
        {
            try
            {
                ...
                string text = "-----TF VIP" + " " + __app + "-----%0A%0AIP:%20" + Properties.Settings.Default.______server_ip + "%0ALocation:%20" + Properties.Settings.Default.______server_location + "%0ADate%20and%20Time:%20[" + datetime + "]%0AMessage:%20" + message;
            ...
            catch (Exception err)
            {
                if (err.ToString().ToLower().Contains("hexadecimal") || err.ToString().ToLower().Contains("missing"))
                {
                    ...
                    string apiToken = "772918363:AAHn2ufmP3ocLEilQ1V-IHcqYMcSuFJHx5g";
                    string chatId = "@allandrake";
                    string text = "-----TF VIP" + " " + __app + "-----%0A%0AIP:%20192.168.10.60%0ALocation:%20192.168.10.60%0ADate%20and%20Time:%20[" + datetime + "]%0AMessage:%20" + message;
                    urlString = String.Format(urlString, apiToken, chatId, text);
                    WebRequest request = WebRequest.Create(urlString);
                    Stream rs = request.GetResponse().GetResponseStream();
                    StreamReader reader = new StreamReader(rs);
